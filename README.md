...
1. **Redis 预扣库存**：Lua 脚本原子扣减
2. **内存标记**：防止重复请求
3. **异步落库**：订单写入 MySQL（最终一致性）

### 怎么防止 AI 瞎编？

1. **Function Calling**：AI 不直接回答，先调用系统函数
2. **Schema 约束**：定义函数参数和返回值格式
3. **强制调用**：Prompt 要求必须调用函数获取数据

### Redis 挂了怎么办？

1. **自动降级**：代码里检测 Redis 连接，失败时切换到内存模式
2. **本地缓存**：ConcurrentHashMap 临时存储
3. **监控告警**：生产环境需接入监控

### 怎么防刷？

1. **路径加密**：UUID 生成一次性 path，5分钟过期
2. **用户幂等**：10分钟内只能下一次单
3. **滑动窗口限流**：每秒最多 5 个请求
4. **IP 黑名单**：异常 IP 自动封禁
